from extra import resource_path
from tkinter import *
...
class LoginPage:
    def __init__(self):
        self.app = "TubeDownloader"
        self.key = b'YR6iNbkOqTE6B9nLzy1r2O9BRmQbNqssOTKlwOLk1v4='
        self.fileKey = b'IOA8WYjyJwm5hxtiww8e4MLfHCvM1aBE6-aA2kQVY5U='
        self.uuid = self.GetUUID()
        self.dataFile = resource_path("data/logs.mpl")
        self.authState = False
        self.version = "9.0.0"
        self.icon = "data/down-arrow.png"
    ...
    def createWindow(self):
        ...
        self.show_button.place(x=845, y=420)
    ...
    '''
        l=Label(r,text=t,bg='grey15',fg='white',font=('',26), anchor="e", justify='left').pack()
        r.mainloop()
    ...
    def show(self):
        ...
        self.hide_button = Button(self.lgn_frame, image=self.show_image, command=self.hide, relief="flat",
                                  activebackground="#040405", fg="#040405"
                                  , borderwidth=0, background="#040405", cursor="hand2")
        self.hide_button.place(x=845, y=420)
    ...
    def login(self, event=None):
        ...
        if self.validateEmail(email):
            if passwd != "":
                ...
                ud = self.encrypt(self.key, self.GetUUID())
                email = self.encrypt(self.key, email)
                passwd = self.encrypt(self.key, passwd)

                try:
                    req = requests_get(f"{self.host}/authapp?action=login&user={email}&pwd={passwd}&ud={ud}&app={self.app}")
                    print("Req==>", req.text)
                except Exception as e:
                    print("e==>",e)
                    if self.isOnline():
                ...
                try:
                    ...
                    #status---user---id--name
                    name = conf[3].split(' ')[0]
                    if conf[1] != email and conf[2] !="89087777766877":
                        self.login_button.configure(text="TryAgain", fg="red")
                        self.passwdVar.set("")
    ...
    def get_auth_key_from_server(self, key=None):
        if key == "info":
            data = f"{genSessionId()}---token-expired---{self.getData()['user']}"
        else:
            data = f"{genSessionId()}---mshare--need--key---{self.getData()['user']}"
            
        req = requests_get(f"{self.host}/authapp?action=getKeys&data={self.encrypt(self.key, data)}&app={self.app}")
        try:
            data = self.decrypt(self.key, req.text.encode())
            data = loads(data.replace("'",'"'))
            print(f"-----> access_token : {data['access_token']}")
            path = resource_path('data/tokens.json')
            ...
            print(f"Error==>get_auth_key_from_server  ==> {e}")
            if key == "info":
                if req.text == "True":
                    return True
    ...
    def forgotPassword(self, event=None):
        ...
        open_file("https://mahadevadity8080.pythonanywhere.com/passwordReset")
    ...
    def writeData(self, data) -> bool:
        #data = {"uuid" : "4C4C4544-0057-4710-8030-B1C04F4A5033", "name" : "mahadev", "user" : "mahadev@gmail.com", "login": time(), "downloads" : []}
        with open(self.dataFile, "wb") as tf:
            tf.write(self.encrypt(self.fileKey, str(data)).encode())
        return 1

    def logout(self):
        data = {"uuid" : "4C4C4544-0057-4710-8030-B1C04F4A5034", "name" : "mahadev", "user" : "mahadev@gmail.com", "login": time(),  "downloads" : []}
        self.writeData(data)
    ...
    def authenticate(self) -> bool:
        if self.getData()["uuid"] == self.uuid:
            return True
        self.createWindow()
        path = sys.executable
        open_file(path)
    ...
    def checkVersion(self):
        ...
        try:
            data = self.getData()
            r = requests_get(f"{self.host}/authapp?action=version&user={self.encrypt(self.key, data['user'])}&app={self.app}")
...
def feedback(event):
    ...
    def sent():
        ...
        st = loginwindow.sendFeedBack(text)
